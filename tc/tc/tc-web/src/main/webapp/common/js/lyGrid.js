...
(function() {
	lyGrid = (function(params) {
		...
		var cBodyTh = function(divid) {
			...
			table2.setAttribute("style", "margin-bottom: 0px;width:"+conf.width); //table-layout: fixed;
						...
						if (!column[o].hide || column[o].hide == undefined) {
							...
							if (l_tree.tree) {
								...
								if(CommnUtil.in_array(lt,clm)){
									...
									img.src=rootPath+"/common/images/tree/nolines_minus.gif";
							...
							var td_o = tr.insertCell(-1);
							td_o.setAttribute("style", "text-align:" + column[o].align + ";width: " + column[o].width + ";display:none;");
		...
		var fenyeDiv = function(divid, jsonData) {
			...
			// TODO ADD LDZ pagesize
			pageSizeObj.html('<option value="'+conf.pageSize+'">'+conf.pageSize+'</option>');
			for ( var i = 0; i < conf.pageSizeArr.length; i++) {
				if(conf.pageSize != conf.pageSizeArr[i]){
					pageSizeObj.append('<option value="'+conf.pageSizeArr[i]+'">'+conf.pageSizeArr[i]+'</option>');
				}
			}
			//conf.pageSize
			lia.innerHTML = '总&nbsp;' + totalRecords + '&nbsp;条&nbsp;&nbsp;每页&nbsp;<span>'+conf.pageSize+'</span>&nbsp;条&nbsp;&nbsp;共&nbsp;' + totalPages + '&nbsp;页';
			
			$(lia).find('span').html(pageSizeObj);
			// TODO 绑定事件
			$(pageSizeObj).change(function(){
				loadData({pageSize:$(pageSizeObj).val()});
			});
			var btd_1 = document.createElement("td");
		...
		var datatree = function() { // 页数
			...
			if(img.src.indexOf("nolines_plus.gif")>-1){
				...
				img.src=rootPath+"/common/images/tree/nolines_plus.gif";
				$("tr[d-tree^='"+ttr+"-']").hide();
			}
		};
		...
		var _getValueByName = function(data, name) {
			if (!data || !name)
				return new Array();
			if (name.indexOf('.') == -1) {
				return data[name]?data[name]:new Array();
			} else {
				try {
		...
		var exportData = function(url,param){
...
var fixhead = function() {
	// 获取表格的宽度
	/*
	 * $('#table_head').css('width',$('.t_table').find('table:first').eq(0).width());
	 */
	for ( var i = 0; i <= $('.t_table .pp-list tr:last').find('td:last').index(); i++) {
		$('.pp-list th').eq(i).css('width', ($('.t_table .pp-list tr:last').find('td').eq(i).width()));
	}
	/*
	 * //当有横向滚动条时，需要此js，时内容滚动头部也能滚动。
	 *  //暂时不处理横向 
	 */
	//$('.t_table').scroll(function() {$('#table_head').css('margin-left', -($('.t_table').scrollLeft())); });
};
